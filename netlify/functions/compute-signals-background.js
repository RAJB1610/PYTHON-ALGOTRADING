// Computes RSI, MACD, Bollinger Bands, MAs, volume ratio, NR7 for all stocks.
// Reads from daily_candles → writes to signals table.
// Run after sync-candles each trading day.
// Optional param: ?date=YYYY-MM-DD (defaults to latest date in daily_candles)

const SB_BATCH    = 500;
const STOCK_BATCH = 50;    // stocks per Supabase candles query
const LOOKBACK    = 365;   // calendar days of history to pull
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // 2 — Get all stocks that have a candle on that date
    const stocks = await getStocksForDate(SUPABASE_URL, SUPABASE_SERVICE_KEY, date);
    if (!stocks.length) throw new Error(`No candles found for date ${date}`);

    // 3 — Process each exchange in batches
    const signals = [];

    for (const exchange of ["NSE", "BSE"]) {
      const syms = stocks.filter(s => s.exchange === exchange).map(s => s.tradingsymbol);

      for (let i = 0; i < syms.length; i += STOCK_BATCH) {
        const batch   = syms.slice(i, i + STOCK_BATCH);
        const candles = await fetchCandles(SUPABASE_URL, SUPABASE_SERVICE_KEY, batch, exchange, cutoff);
    ...
    // 4 — Upsert to signals table
    for (let i = 0; i < signals.length; i += SB_BATCH) {
      const res = await fetch(`${SUPABASE_URL}/rest/v1/signals`, {
        ...
        body: JSON.stringify(signals.slice(i, i + SB_BATCH))
      });
      if (!res.ok) {
        const err = await res.text();
        throw new Error(`Supabase signals upsert failed: ${err.slice(0, 300)}`);
      }
    }
    ...
    return {
      statusCode: 200,
      headers: H,
      body: JSON.stringify({ ok: true, date, computed: signals.length, total_stocks: stocks.length })
    };
  } catch (e) {
...
async function getStocksForDate(url, key, date) {
  const all = [], PAGE = 5000;
  let offset = 0;
  while (true) {
    const res  = await sbGet(url, key,
      `/rest/v1/daily_candles?select=tradingsymbol,exchange&date=eq.${date}&limit=${PAGE}&offset=${offset}`);
    const data = await res.json();
...
async function fetchCandles(url, key, symbols, exchange, cutoff) {
  const inList = symbols.map(s => encodeURIComponent(s)).join(",");
  const all = [], PAGE = 20000;
  let offset = 0;
  while (true) {
    const res  = await sbGet(url, key,
      `/rest/v1/daily_candles?select=tradingsymbol,date,close,high,low,volume` +
      `&tradingsymbol=in.(${inList})&exchange=eq.${exchange}&date=gte.${cutoff}` +
...
function computeSignals(candles) {
  ...
  const isNR7  = ranges.length >= 7
    && ranges.at(-1) < Math.min(...ranges.slice(-7, -1));
...
function calcMACD(closes, fast = 12, slow = 26, signal = 9) {
  ...
  return {
    value:        +curM.toFixed(4),
    signal:       +curS.toFixed(4),
    hist:         +(curM - curS).toFixed(4),
...
function calcBB(closes, period = 20, mult = 2) {
  ...
  const upper = +(mean + mult * std).toFixed(4);
  const lower = +(mean - mult * std).toFixed(4);
  const cmp   = closes.at(-1);
  ...
  return { upper, lower, position: pos, isSqueeze: width < avgW * 0.8 };
}
